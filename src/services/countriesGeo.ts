...
export interface CountryZoomProfile {
  altitude: number;
  ringMaxRadius: number;
  isSmall: boolean;
}

// Micro-states and small island territories requiring close-up zoom
const MICRO_STATES: Set<string> = new Set([
  'VA', 'MC', 'NR', 'TV', 'SM', 'LI', 'MH', 'KN', 'MV', 'MT',
  'GD', 'VC', 'BB', 'AG', 'SC', 'PW', 'AD', 'LC', 'FM', 'SG',
  'TO', 'DM', 'BH', 'KI', 'ST', 'MU', 'KM', 'WS', 'CV', 'LU',
  'TT', 'BN', 'TC', 'KY', 'BM', 'VG', 'VI', 'AI', 'MS', 'AW',
  'CW', 'SX', 'BQ', 'BL', 'MF', 'PM', 'WF', 'CK', 'NU', 'TK',
  'NF', 'SH', 'GI', 'JE', 'GG', 'IM', 'FO', 'AX',
]);

// Small island & mainland nations requiring medium-close zoom
const SMALL_COUNTRIES: Set<string> = new Set([
  'CY', 'LB', 'QA', 'JM', 'XK', 'GM', 'ME', 'VU', 'BS', 'TL',
  'SZ', 'KW', 'FJ', 'SI', 'IL', 'SV', 'BZ', 'DJ', 'MK', 'RW',
  'BI', 'GQ', 'HT', 'DO', 'SB', 'AL', 'AM', 'LS', 'BE', 'MD',
  'NL', 'CH', 'DK', 'EE', 'LV', 'LT', 'CR', 'PA', 'PR', 'TW',
]);

// Medium sized countries
const MEDIUM_COUNTRIES: Set<string> = new Set([
  'GB', 'IT', 'DE', 'JP', 'PH', 'NZ', 'CU', 'GH', 'UG', 'KE',
  'EC', 'UY', 'PY', 'BO', 'CL', 'PE', 'CO', 'VE', 'GY', 'SR',
  'NO', 'SE', 'FI', 'PL', 'RO', 'BG', 'GR', 'TR', 'ES', 'PT',
  'MA', 'TN', 'DZ', 'EG', 'SN', 'CI', 'NG', 'CM', 'AO', 'ZA',
  'MZ', 'ZW', 'BW', 'NA', 'TZ', 'MG', 'SO', 'ET', 'SD', 'SS',
  'IQ', 'IR', 'SA', 'YE', 'OM', 'AE', 'JO', 'SY', 'PK', 'AF',
  'UZ', 'TM', 'TJ', 'KG', 'KZ', 'MN', 'NP', 'BT', 'BD', 'MM',
  'TH', 'LA', 'VN', 'KH', 'MY', 'ID', 'PG',
]);

/**
 * Returns the optimal camera zoom altitude and ring radius for a given country.
 * For small islands and micro-states, gives a close-up camera view so the player
 * can easily see the country's detailed geography and coastline shape.
 */
export function getCountryZoomProfile(alpha2: string): CountryZoomProfile {
  const upper = (alpha2 || '').toUpperCase();
  if (MICRO_STATES.has(upper)) {
    return {
      altitude: 0.38,
      ringMaxRadius: 1.1,
      isSmall: true,
  ...
  // Giant continental countries (US, CA, RU, CN, BR, AU, IN, etc.)
  return {
    altitude: 1.85,
    ringMaxRadius: 5.2,
    isSmall: false,
